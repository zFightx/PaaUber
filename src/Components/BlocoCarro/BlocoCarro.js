...
const BlocoCarro = ({
  ...
  const RenderClientes = () => {
    return listaClientes.map((cliente) => (
      <div
        className="opcaoCliente"
        key={cliente.id}
        onMouseEnter={() => {
          DesenharCaminho(cliente.resultado.caminho, [cliente.id, carro.id]);
        }}
        onMouseLeave={() => {
          ApagarCaminho(cliente.resultado.caminho);
        }}
  ...
  const VerClientes = (withTime) => {
    ...
    carro.tem_cliente.forEach((clienteId) => {
      ...
      const start =
        vertices[verticeMaisProximo(carro.loc.x, carro.loc.y, vertices)];
      const end =
        vertices[verticeMaisProximo(cliente.loc.x, cliente.loc.y, vertices)];
      const dest =
        vertices[verticeMaisProximo(cliente.dest.x, cliente.dest.y, vertices)];
  ...
  const VerCaminhos = () => {
    ...
    const end =
      vertices[
        verticeMaisProximo(
          clientes[clienteSelect.id].loc.x,
          clientes[clienteSelect.id].loc.y,
          vertices
        )
      ];
  ...
  const VerTrajetos = () => {
    ...
    const end =
      vertices[
        verticeMaisProximo(
          clientes[clienteSelect.id].dest.x,
          clientes[clienteSelect.id].dest.y,
          vertices
        )
      ];
  ...
  const RenderCaminhos = (secondColor) => {
    return listaCaminhos.map((caminho, index) => (
      <div
        ...
        onMouseEnter={() => {
          DesenharCaminho(
            caminho.resultado.vertices,
            secondColor ? [clienteSelect.id] : [clienteSelect.id, carro.id],
            secondColor
          );
        }}
        onMouseLeave={() => {
          ApagarCaminho(caminho.resultado.vertices);
        }}
  ...
  const AceitarCorrida = () => {
    ...
    setDadosCorridas((dadosAtuais) => [
      ...dadosAtuais,
      { tempoCorrida: tempoCorrida, tempoEspera: tempoEspera, dist: dist },
    ]);
  ...
  return (
    <div
      className={`blocoCliente blocoCarro ${addClass}`}
      onMouseLeave={ApagarCaminho}
    >
      ...
      {!carro.ocupado && subPage == 0 && (
        <>
          <div className="opcaoCliente" onClick={() => VerClientes(false)}>
            <div className="opcaoText">
          ...
          <div className="opcaoCliente" onClick={() => VerClientes(true)}>
            <div className="opcaoText">
              <p>Ver Solicitações</p>
              <p>por tempo</p>
            </div>
          ...
          <div className="opcaoCliente" onClick={() => DeletarCarro(carro.id)}>
            <div className="opcaoText">
              <p>Excluir</p>
      ...
      {!carro.ocupado && subPage == 2 && (
        <>
            ...
            {comTempo && (
              <>
                ...
                <p>
                  Total:{" "}
                  {(
                    clienteSelect.resultado.tempo +
                    clienteSelect.resultado2.tempo
                  ).toFixed(2)}{" "}
                  horas
                </p>
              </>
            ...
            {!comTempo && (
              <>
                ...
                <p>
                  Distância Total:{" "}
                  {(
                    clienteSelect.resultado.dist + clienteSelect.resultado2.dist
                  ).toFixed(2)}{" "}
                  km
                </p>
              </>
          ...
          <div
            className="opcaoCliente"
            onMouseEnter={() => {
              ApagarCaminho(clienteSelect.resultado2.caminho);
              DesenharCaminho(
                clienteSelect.resultado.caminho,
                [clienteSelect.id, carro.id],
                false
              );
            }}
            onMouseLeave={() => {
              ApagarCaminho(clienteSelect.resultado.caminho);
            }}
          >
            <div className="opcaoText">
          ...
          <div
            ...
            onMouseEnter={() => {
              ApagarCaminho(clienteSelect.resultado.caminho);
              DesenharCaminho(
                clienteSelect.resultado2.caminho,
                [clienteSelect.id],
                true
              );
            }}
          ...
          <div className="opcaoCliente" onClick={() => VerCaminhos()}>
            <p>Outras opções de ir até o cliente</p>
            <i className="fas fa-angle-right"></i>
          </div>

          <div className="opcaoCliente" onClick={() => VerTrajetos()}>
            <p>Outras opções de trajetos até o destino</p>
            <i className="fas fa-angle-right"></i>
          </div>

          <div className="opcaoButton" onClick={() => AceitarCorrida()}>
            <p>Aceitar Corrida</p>
